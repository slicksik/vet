...
const Navbar: React.FC = () => {
  ...
  const pendingBookingsCount =
    user?.role === "vet"
      ? bookings.filter((b) => b.vetId === user?.id && b.status === "pending")
          .length
      : 0;
        ...
        <div className="flex justify-between items-center h-16">
          <div className="flex items-center gap-12">
            ...
            <Link to="/" className="flex items-center group gap-3">
              ...
              <span
                className={`text-2xl font-bold tracking-tight transition-colors duration-300 ${
                  isScrolled || !isHome ? "text-gray-900" : "text-gray-900" // Keeping text dark for readability on hero
                }`}
              >
                Vetify
              </span>
            ...
            <div className="hidden md:flex items-center space-x-1">
              {[
                { name: "Αρχική", path: "/" },
                { name: "Βρείτε κτηνίατρο", path: "/search" },
                { name: "Για κτηνιάτρους", path: "/for-vets" },
              ].map((link) => (
                <Link
                  key={link.name}
                  to={link.path}
